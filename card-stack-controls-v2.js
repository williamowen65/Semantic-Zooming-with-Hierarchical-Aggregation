...
(() => {
  ...
  function rootCard(y){
    stage.selectAll('.root-context-entry').remove();const b=box(),ch=width<720?100:82;
  ...
  function layout(animate=false){
    ...
    y+=rootCard(y)+12;const root=rootLayer();if(rootOpen&&root){path.length?root.classList.add('card-stack-layer-visible'):root.classList.remove('card-stack-layer-hidden');setY(root,y,animate);y+=h(root)+12;}else if(!path.length&&root)root.classList.add('card-stack-layer-hidden');
    entries.forEach((entry,i)=>{const id=path[i],current=i===entries.length-1,fo=wire(entry,id,current);if(!id||!fo)return;entry.removeAttribute('transform');fo.removeAttribute('transform');setCardY(fo,y,animate);const ch=+fo.getAttribute('height')||66,node=nodeById.get(id),hasChildren=(node?.children||[]).length>0,open=(current&&hasChildren)||(visible.get(id)===true&&hasChildren),layer=layerFor(i,entries.length-1),toggle=entry.querySelector('foreignObject.layer-kind-toggle-host');const th=toggle?(+toggle.getAttribute('height')||24):0;if(toggle){toggle.removeAttribute('transform');toggle.style.display='';d3.select(toggle).attr('y',y+ch+4);}if(open&&layer){i<entries.length-1?layer.classList.add('card-stack-layer-visible'):layer.classList.remove('card-stack-layer-hidden');const top=y+ch+4+th/2;setY(layer,top,animate);y=top+h(layer)+12;}else{if(i===entries.length-1&&layer)layer.classList.add('card-stack-layer-hidden');y+=ch+(toggle?th+8:10);}});
