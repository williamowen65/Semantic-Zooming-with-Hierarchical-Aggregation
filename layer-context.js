...
// space between that level and the next one. This replaces the floating topic
// bottom sheet so the context travels with the hierarchy itself.
(() => {
  ...
  const affectedLocationCount = node => {
    ...
    // Temporary deterministic demo value until location entities are added to
    // the dataset. Explicit location data automatically takes precedence.
  ...
  function renderLayerContextEntries() {
    ...
    focusPath.forEach((id, index) => {
      ...
      let gapTop = currentBottom, gapBottom;

      if (next) {
        gapBottom = parseTranslateY(next) + next.getBBox().y;
      } else {
        gapBottom = currentBottom + (width < 720 ? 82 : 96);
      }

      const available = Math.max(42, gapBottom - gapTop);
      const cardHeight = Math.min(width < 720 ? 58 : 48, Math.max(42, available - 16));
      ...
      const html = `<div xmlns="http://www.w3.org/1999/xhtml" class="layer-context-card ${node.kind === 'solution' ? 'is-solution' : 'is-issue'}"><div class="layer-context-primary"><span class="layer-context-kind">${node.kind === 'solution' ? 'Solution' : 'Issue'}</span><span class="layer-context-name">${esc(node.name)}</span></div><div class="layer-context-stats">${locationHtml}<span class="layer-context-stat"><strong>${compact(node.votes)}</strong> votes</span><span class="layer-context-stat"><strong>${Number(node.rating || 0).toFixed(1)}</strong> avg</span><span class="layer-context-stat"><strong>${childCount}</strong> children</span></div></div>`;
  ...
  // enhancements.js already performs one render before this file loads, so
  // populate the inter-level context once immediately as well.
  renderLayerContextEntries();
})();