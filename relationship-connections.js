...
(() => {
  ...
  function decorateRelationshipNodeLabels(){
    d3.selectAll('#viz g.cell').each(function(d){
      ...
      const parts=contextualRelationshipParts(item);
      cell.append('foreignObject').attr('class','relationship-node-label-host').attr('x',bounds.x+padX).attr('y',bounds.y+padY).attr('width',w).attr('height',h)
        .html(`<div xmlns="http://www.w3.org/1999/xhtml" class="relationship-node-label"><div class="relationship-node-title"><span class="relationship-node-keyword">${esc(parts.label)}</span> <span class="relationship-node-endpoint">${esc(parts.other)}</span></div><div class="relationship-node-meta">${meta.map(line=>`<span>${esc(line)}</span>`).join('')}</div></div>`);
    });
  }
