(() => {
  ...
  window.renderAtlasDiagram = function(container, items, options={}) {
    ...
    const width=Math.max(320,container.clientWidth||900),height=Math.max(280,Math.min(520,width*.52));
    ...
    cells.append('path').attr('d',d=>polygonPath(d.polygon)).attr('fill',(d,i)=>d3.interpolateRgb('#dfe9ee','#9eb9c7')(items.length===1?.4:i/Math.max(1,items.length-1)));
    cells.each(function(d){const [cx,cy]=d3.polygonCentroid(d.polygon),area=Math.abs(d3.polygonArea(d.polygon)),font=Math.max(10,Math.min(17,Math.sqrt(area)/10));const lines=fitLabel(d.data.item.title,Math.max(16,Math.round(Math.sqrt(area)/5.5)));const text=d3.select(this).append('text').attr('x',cx).attr('y',cy-(lines.length-1)*font*.55).attr('text-anchor','middle').style('font-size',`${font}px`);lines.forEach((line,i)=>text.append('tspan').attr('x',cx).attr('dy',i===0?0:font*1.1).text(line));text.append('tspan').attr('class','small').attr('x',cx).attr('dy',font*1.35).style('font-size',`${Math.max(9,font*.72)}px`).text(d.data.item.type);});
  };
})();
