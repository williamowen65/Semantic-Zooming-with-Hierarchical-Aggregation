(() => {
  ...
  function adjust() {
    ...
    // The visual stack was moved down, so the camera's scrollable content bounds
    // must move down by the same amount. Desktop wheel scrolling is clamped by
    // worldHeight; without this adjustment it can get stuck before reaching cards.
    if (typeof worldHeight === 'number' && Number.isFinite(worldHeight)) {
      worldHeight += delta;
    }
