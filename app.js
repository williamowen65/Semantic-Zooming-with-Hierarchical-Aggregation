// Atlas root-only seed. There is intentionally no child hierarchy yet.
const forestData = [
  { id:"root-health", name:"Health & Wellbeing", votes:9800, rating:4.7, kind:"issue", color:"#71879a" },
  { id:"root-economy", name:"Money, Work & Economy", votes:9400, rating:4.6, kind:"issue", color:"#71879a" },
  { id:"root-housing", name:"Housing & Built Environment", votes:9000, rating:4.7, kind:"issue", color:"#71879a" },
  { id:"root-education", name:"Education & Knowledge", votes:8500, rating:4.6, kind:"issue", color:"#71879a" },
  { id:"root-environment", name:"Environment & Natural World", votes:9300, rating:4.8, kind:"issue", color:"#71879a" },
  ...
  { id:"root-government", name:"Government & Public Institutions", votes:8300, rating:4.5, kind:"issue", color:"#71879a" },
  ...
  { id:"root-technology", name:"Technology & Information", votes:8800, rating:4.7, kind:"issue", color:"#71879a" },
...
function wrapLabel(text,maxChars){const words=text.split(/\s+/),lines=[];let line="";words.forEach(word=>{const next=line?`${line} ${word}`:word;if(next.length>maxChars&&line){lines.push(line);line=word;}else line=next;});if(line)lines.push(line);return lines.slice(0,4);}

function renderCluster({items,x,y,w,h,selectedId=null,faded=false,interactive=true,className=""}){
  const {root,polygon}=layoutCluster(items,w,h,items.map(d=>d.id).join("-"));
  const g=stage.append("g").attr("class",`cluster ${className}`).attr("transform",`translate(${x},${y})`);
  g.append("path").attr("class","cluster-outline").attr("d",polygonPath(polygon));
  const leaves=root.leaves();
  const cells=g.selectAll("g.cell").data(leaves,d=>d.data.id).join("g")
    .attr("class",d=>`cell ${d.data.id===selectedId?"is-selected":""} ${faded&&d.data.id!==selectedId?"is-faded":""}`)
    ...
    .on("click",(event,d)=>{if(!interactive||touchMoved)return;event.stopPropagation();focusNode(d.data.item.id);})
    .on("keydown",(event,d)=>{if(interactive&&(event.key==="Enter"||event.key===" ")){event.preventDefault();focusNode(d.data.item.id);}});
...
function focusNode(id){const node=nodeById.get(id);if(!node)return;focusPath=pathForNode(id);render();scrollToDepth(focusPath.length-1,true);statusHost.textContent=`${node.name} selected. No child nodes have been added yet.`;}
...
function render(){
  width=host.clientWidth;height=host.clientHeight;svg.attr("viewBox",[0,0,width,height]);stage.selectAll("*").remove();levelCenters=[];renderBreadcrumbs();
  const compactMobile=width<720,contentTop=compactMobile?132:98,centerX=width/2,geometry=levelGeometry(compactMobile,contentTop);
  if(!focusPath.length){cameraY=0;worldHeight=height;stage.attr("transform","translate(0,0)");renderCluster({items:forestData,x:0,y:contentTop,w:geometry.w,h:geometry.h,interactive:true,className:"root-overview"});stage.append("text").attr("class","canvas-caption").attr("x",centerX).attr("y",Math.min(height-22,contentTop+geometry.h+32)).attr("text-anchor","middle").text("Broad Atlas roots · child hierarchy not populated yet");return;}
  const selected=currentNode(),cursorY=contentTop;
  const rendered=renderCluster({items:forestData,x:0,y:cursorY,w:geometry.w,h:geometry.h,selectedId:selected.id,faded:true,interactive:true,className:"context-cluster depth-0"});
  levelCenters.push(cursorY+geometry.h/2);
  const point=selectedCentroid(rendered,selected.id,0,cursorY);
  if(point) stage.append("text").attr("class","leaf-message").attr("x",centerX).attr("y",cursorY+geometry.h+58).attr("text-anchor","middle").text("No child nodes yet · use All roots or select another root");
  worldHeight=cursorY+geometry.h+118;applyCamera(false);
}
