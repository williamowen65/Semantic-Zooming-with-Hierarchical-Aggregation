...
function render(){
  ...
  if(!focusPath.length){cameraY=0;worldHeight=height;stage.attr("transform","translate(0,0)");renderCluster({items:forestData,x:0,y:contentTop,w:geometry.w,h:geometry.h,interactive:true,className:"root-overview"});return;}
  ...
  const selected=currentNode(),children=selected?.children||[];if(children.length){renderCluster({items:children,x:0,y:cursorY,w:geometry.w,h:geometry.h,interactive:true,className:"child-cluster"});levelCenters.push(cursorY+geometry.h/2);cursorY+=geometry.h+gap;}else{stage.append("text").attr("class","leaf-message").attr("x",centerX).attr("y",cursorY-30).attr("text-anchor","middle").text("No child nodes yet");}
