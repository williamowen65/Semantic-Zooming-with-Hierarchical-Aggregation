(() => {
  ...
  function makeCard(node,{root=false}={}){const card=document.createElement('article');card.className=root?'root-card':'node-card';card.dataset.cardId=node.id;const head=document.createElement('div');head.className='node-head';head.append(typeBadge(node.type));const title=document.createElement('h2');title.textContent=node.title;head.append(title);card.append(head);const desc=document.createElement('p');desc.className='description';desc.textContent=node.description||'';card.append(desc);const meta=document.createElement('div');meta.className='meta';meta.textContent=`${children(node).length} direct responses · requests ${requested(node).length} response type${requested(node).length===1?'':'s'}`;card.append(meta);const related=linksByNode.get(node.id)||[];if(related.length){const wrap=document.createElement('div');wrap.className='crosslinks';for(const rel of related){const otherId=rel.from===node.id?rel.to:rel.from,other=nodes.get(otherId);if(!other)continue;const button=document.createElement('button');const label=rel.from===node.id?rel.label:`Related via ${rel.label.toLowerCase()}`;button.innerHTML=`<strong>${escapeHtml(label)}</strong> ${escapeHtml(other.title)}`;button.onclick=()=>navigateTo(other.id);wrap.append(button);}card.append(wrap);}return card;}
  ...
  function render(){renderBreadcrumbs();app.replaceChildren();const stack=document.createElement('div');stack.className='stack';const rootSummary={id:'__roots',type:'collection',title:'Root challenges',description:'Starting points can use different types. Root nodes are shown together as an intentional exception to ordinary parent-requested type filtering.',requestedChildTypes:[]};const rootCard=makeCard(rootSummary,{root:true});rootCard.querySelector('.meta').textContent=`${data.rootIds.length} roots · ${new Set(data.rootIds.map(id=>nodes.get(id)?.type).filter(Boolean)).size} types`;stack.append(rootCard,layerFor(null,{root:true}));for(const id of path){const node=nodes.get(id);if(!node)continue;stack.append(makeCard(node));if(requested(node).length)stack.append(makeResponseBar(node),layerFor(node));}app.append(stack);}
