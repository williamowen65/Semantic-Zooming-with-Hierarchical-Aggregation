...
The **Next planning** links are deliberately actionable: they point to the planning document where the remaining work should be resolved, so the dashboard can be used as a navigation surface rather than only a status report.

| Area | Coverage | Status | Current read / next planning |
|---|---:|:---:|---|
| [Graph and Content Model](graph/README.md) | ~75% | 🟢 | Most-developed domain. Core Node model, emergent ontology, requested child types, relationships, multi-parent convergence, repository/caching direction, version history, navigation, permission boundaries, and the broader Graph security surface have been explored. **Next:** resolve [authorization, relationship permissions, abuse resistance, concurrency, visibility, event integrity, and authorization-aware caching](graph/permissions-and-ownership.md), plus remaining [persistence decisions](graph/persistence-model.md) and consolidated [open Graph decisions](graph/open-decisions.md). |
| [Voting](voting/README.md) | ~20% | 🔴 | Strategy-pattern direction exists for scoring/ranking, but the meaning of votes, scoring rules, persistence, abuse handling, and flows remain sparse. **Next:** work through the [Voting planning area](voting/README.md), using its domain model, behavior, persistence, permissions, flows, and open decisions as the checklist. |
| [Profiles and Identity](profiles-and-identity/README.md) | ~20% | 🔴 | Multiple profile roots and contextual root status are known. Ownership, identity rules, privacy, and profile behavior need substantial design. **Next:** continue in [Profiles and Identity](profiles-and-identity/README.md), especially its domain model, permissions/ownership, behavioral rules, and flows. |
| [Moderation](moderation/README.md) | ~10% | 🔴 | Mostly a planning scaffold. Governance, moderation states, authority, reporting, auditability, and shared-node consequences remain open. **Next:** develop the [Moderation planning area](moderation/README.md), beginning with its domain/UML and authority/permissions model. |
| [Notifications](notifications/README.md) | ~30% | 🟡 | Initial domain model, domain events, Observer/Publish-Subscribe, delivery adapters, key flows, and persistence questions are documented. **Next:** resolve recipient rules, preferences, grouping, retry/idempotency, and milestone scope in [Notifications](notifications/README.md). |
| [Architecture](architecture/README.md) | ~65% | 🟢 | Modular bounded contexts, contracts/events, per-context DbContexts over shared SQL, client-code sketches, Graph permission-boundary interaction, and Graph query/command/event direction are established. **Next:** resolve event dispatch/durable messaging and transaction boundaries in [Codebase Structure and Bounded Contexts](architecture/codebase-structure-and-bounded-contexts.md); refine the C# ↔ Python boundary in [Application Architecture](architecture/application-architecture.md); and settle deployment/runtime topology in [Infrastructure and Deployment](architecture/infrastructure-and-deployment.md). |
| [Security](security/README.md) | ~20% | 🔴 | Important concerns are identified, but concrete authorization, validation, service security, threat modeling, and abuse protections need deeper design. **Next:** use the [Security planning area](security/README.md) to turn those concerns into explicit boundaries and rules; Graph-specific security stays in [Graph Permissions and Ownership](graph/permissions-and-ownership.md). |
| [Product Experience](product-experience/README.md) | ~35% | 🟡 | The prototype produced substantial interaction knowledge, but it has not yet been systematically converted into rewrite-oriented screen and behavior specifications. **Next:** translate prototype lessons into explicit rewrite requirements in [Product Experience](product-experience/README.md). |
| [Rewrite Execution](rewrite/README.md) | ~35% | 🟡 | Clean-rewrite strategy, old-code salvage, test review, and minimum-milestone thinking exist. **Next:** finalize the first implementation milestone and migration/salvage decisions in [Rewrite Execution](rewrite/README.md). |
