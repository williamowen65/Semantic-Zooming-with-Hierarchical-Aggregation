...
| [Architecture](architecture/README.md) | ~65% | 🟢 | Modular bounded contexts, contracts/events, per-context DbContexts over a shared SQL database, client-code/use-case sketches, Graph permission-boundary interaction, and the Graph query/command/event contract direction are established. Event dispatch/durable messaging, transaction boundaries, the C# ↔ Python service contract, deployment/runtime topology, and operational details remain. |
