// Controls only the vertical height of each hierarchy layer.
// No viewport zooming, scaling, width emulation, or interface resizing is used.
(() => {
  if (typeof render !== 'function' || typeof levelGeometry !== 'function') return;

  const minus = document.querySelector('#layer-height-down');
  const plus = document.querySelector('#layer-height-up');
  const readout = document.querySelector('#layer-height-readout');
